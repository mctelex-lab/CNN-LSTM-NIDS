...
# Display TensorFlow and NumPy info
with st.expander("ℹ️ System Information", expanded=False):
st.write(f"**TensorFlow Version:** {tf.__version__}")
st.write(f"**NumPy Version:** {np.__version__}")
st.write(f"**Python Version:** {tf.sys.version_info.major}.{tf.sys.version_info.minor}")
...
st.error("❌ No models loaded! Please ensure all deployment artifacts are present.")
st.stop()
...
with tab1:
    ...
    if uploaded_file:
        ...
        # Show first few features expected
        with st.expander("📋 Expected Features"):
            st.write(f"Expected {len(feature_list)} features. First 10: {feature_list[:10]}")
        ...
        if missing_features:
            ...
            if st.button("🔍 Analyze Traffic", type="primary"):
                with st.spinner("Analyzing network traffic with CNN-LSTM model..."):
                    try:
                        ...
                        df['risk_level'] = np.where(pred_proba >= 0.7, 'High', 
                                                    np.where(pred_proba >= 0.5, 'Medium', 'Low'))
                        
                        st.subheader("📋 Detection Results")
                        st.dataframe(df[['prediction', 'confidence', 'risk_level'] + feature_list[:5]], 
        ...
        if os.path.exists(lime_dir):
            ...
            if lime_files:
                ...
                **📖 LIME Interpretation Guide:**
                ...
                st.info("No LIME visualization files found. Run training pipeline to generate.")
        else:
...
with tab3:
    ...
    obj_status = [
        ...
        ("✅ Objective ii: Dual XAI Integration" if xai_available else "❌ Objective ii: Dual XAI Integration - XAI FILES MISSING", xai_available),
...
with tab4:
    ...
    **Title:** Explainable Hybrid 1D CNN-LSTM Framework for Real-Time Intrusion Detection
    
    **Developer:** Samuel Ayorinde A
    **Program:** PGD in Cybersecurity
    **Institution:** Nigerian Defence Academy (NDA), Kaduna, Nigeria
    **Supervisor:** Mr Victor Akuboh
    ...
    |-----------|-------------|--------|
    | **i** | Lightweight CNN-LSTM architecture (<500K params) | ✅ |
    | **ii** | Dual XAI Integration (LIME + SHAP) | ⚠️ Requires XAI files |
    | **iii** | Data balancing with SMOTE | ✅ |
    | **iv** | Cloud-native Streamlit dashboard | ✅ |
    | **v** | Comprehensive evaluation metrics | ✅ |
    
    ### 📊 Datasets Used
    
    **CIC-IDS-2018**
    - 3-class classification (Benign, FTP-BruteForce, SSH-Bruteforce)
    - Network flow features
    - Sampled 150,000 flows
